...
parser.add_argument(
    "--mask",
    type=str,
    default="./datasets/XDS/mask.npy",
    help="mask npy file path",
)
parser.add_argument(
    "--fileExt" ,
    type=str,
...
def ascGenMaskAndPredictClean(file_path, predict, mask):
...
def npArray2asc(header, imgData, file_path):
    ...
    imgData = np.where(imgData == "99999.123", newNODATA_value, imgData)
...
image_size = (1793,1334)  #不同區域大小可能不同
pad_size = (8,8,4,3)
device = "cpu"
dtype = float
loss = nn.MSELoss()
...
for file in files:
    ...
    input_data = torch.cat((basic,torch.ones(input_sample.shape)*int(re.findall(f"(\d+).{opt.fileExt}",file)[0]), input_sample),dim=0).unsqueeze(0).to(device,dtype=dtype)
    # print("input_data.shape: ",input_data.shape, "findall: ",re.findall("(\d+).asc",file)[0], "file: ",file)
    model.eval()
    predict = model(input_data).cpu().detach()
    predict = ascGenMaskAndPredictClean(file,predict.numpy().reshape(image_size),mask)
